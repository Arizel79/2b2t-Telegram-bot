...
async def handler_kills_top_month_callback(self, callback: CallbackQuery) -> None:
    await self.on_event(callback)

    try:
        spl = callback.data.split()
        assert spl[0] == CALLBACK_KILLS_TOP_MONTH
        if spl[1] == "none":
        ...
        if spl[2] == "goto":
            ...
            pages_count = await self.api_2b2t.get_kills_top_month_pages_count(q_data["page_size"])
